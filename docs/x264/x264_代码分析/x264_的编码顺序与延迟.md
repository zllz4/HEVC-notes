...
1. 单个视频帧的顺序表示变量
    ...
    - `x264_frame_t.i_frame_num`: 该帧的**参考顺序**（瞎起的名），由 H264 官方文档的 7.4.3 小节定义
        ...
        ![x264_的输入顺序、编码顺序、输出顺序与延迟_4766373888](markdown_images/x264_%E7%9A%84%E8%BE%93%E5%85%A5%E9%A1%BA%E5%BA%8F%E3%80%81%E7%BC%96%E7%A0%81%E9%A1%BA%E5%BA%8F%E3%80%81%E8%BE%93%E5%87%BA%E9%A1%BA%E5%BA%8F%E4%B8%8E%E5%BB%B6%E8%BF%9F_4766373888.png)
...
2. 编码器状态中的顺序表示变量
    ...
    - `x264_t.i_frame`: 编码器的**编码次数统计**，`x264_encoder_encode` 函数每编码一帧，其 `h->i_frame` 加 1，由于其可以表示编码顺序，这一统计结果能够用来给重建帧 `h->fdec`（其为 `x264_frame_t` 结构体）的 `i_dts` 成员进行赋值
